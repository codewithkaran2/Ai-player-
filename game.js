...
// (Volume slider code removed – using default volume settings)
...
/* 
  ...
  The AI now uses a higher movement factor (0.2),
  shoots from farther away (distance threshold increased to 250),
  and its shooting cooldown is reduced to 100ms.
*/
function updateAI() {
  if (gameMode === "solo") {
    ...
    let factor = 0.2; // increased for more aggressive movement
    ...
    if (distance < 250 && player2.canShoot && gameRunning && !gamePaused) {
      shootBullet(player2, 2);
      player2.canShoot = false;
      setTimeout(() => { player2.canShoot = true; }, 100);
    }
  }
}
...
function drawTopStatus() {
  ...
  // Name boxes (adjusted to center text within each box)
...
function checkGameOver() {
  if (player1.health <= 0 || player2.health <= 0) {
    ...
    // Score display updated inside canvas now
    showGameOverScreen();
  }
}
